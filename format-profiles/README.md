...
## Page geometry during layout

The table's “File extent” is the calculated trim-plus-bleed rectangle, excluding printer marks or other export margins; it does not prescribe a PDF MediaBox. In a layout application with separate bleed controls, set document pages to trim dimensions and configure bleed separately. Confirm actual PDF page boxes, marks and any binding-specific gutter treatment with the printer. The safe inset and bleed examples do not replace those requirements.
